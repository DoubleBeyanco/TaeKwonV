...
namespace UnityEngine.XR.Content.Interaction
{
    ...
    public class XRSlider : XRBaseInteractable
    {
        ...
        void UpdateSliderPosition()
        {
            ...
            var newSliderValue = Mathf.Clamp01((localPosition.z - m_MinPosition) / (m_MaxPosition - m_MinPosition));

            // Use Lerp to smoothly transition from the current slider value to the new slider value
            var smoothSliderValue = Mathf.Lerp(m_Value, newSliderValue, Time.deltaTime);

            SetValue(smoothSliderValue);
            SetSliderPosition(smoothSliderValue);
        }
